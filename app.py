...
NEWS = [
    {
        ...
        "kind": "fix",
        "text": {
            "ru": "**Багфиксы по итогам аудита.** Исправлено: при разных валютах внутренние поля "
                  "расчёта вывода средств больше не смешивают валюты; курс 0 теперь предупреждает "
                  "(раньше молча обнулял все суммы); загрузка шаблона коллекции защищена от битых "
                  "данных (качество вне резки роняло приложение); округление цен для целочисленных "
                  "валют устойчиво к погрешности дробных чисел (14.5 → 15). Плюс: флоат в таблице "
                  "кандидатов показывается точнее (6 знаков), в Режиме 4 подписано, к какой валюте "
                  "приведены суммы.",
            "en": "**Bug-fix release (audit follow-up).** Fixed: cash-out internals no longer mix "
                  "currencies in cross-currency mode; a rate of 0 now warns instead of silently "
                  "zeroing every figure; collection-template loading is now guarded against bad data "
                  "(a quality outside the float cap used to crash the app); price rounding for "
                  "integer currencies is now immune to floating-point error (14.5 → 15). Also: "
                  "6-decimal floats in the candidates table, and Mode 4 now states which currency "
                  "the figures are converted to.",
            "ua": "**Багфікси за підсумками аудиту.** Виправлено: за різних валют внутрішні поля "
                  "розрахунку виведення коштів більше не змішують валюти; курс 0 тепер попереджає "
                  "(раніше мовчки обнуляв усі суми); завантаження шаблону колекції захищене від "
                  "битих даних (якість поза різанням роняла застосунок); заокруглення цін для "
                  "цілочисельних валют стійке до похибки дробових чисел (14.5 → 15). Плюс: флоат у "
                  "таблиці кандидатів точніший (6 знаків), у Режимі 4 підписано, до якої валюти "
                  "приведені суми.",
        },
    },
    {
